...
function seedAttendance(dates) {
  ...
  seedStudents.forEach((st) => {
    ...
    subs.forEach((sub) => {
      dates.forEach((date) => {
        const key = `${st.id}__${sub.id}__${date}__1`;
...
export default function AttendancePortal() {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        const res = await fetch("/api/state", {
          ...
          body: JSON.stringify({ subjects, students, attendance, sessions, attendanceMeta }),
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        setSyncState("saved");
      } catch (err) {
  ...
  const editStudent = (oldId, data) => {
    ...
    if (newId !== oldId) {
      ...
      // they still point at this student
      const remapKey = (key) => {
  ...
  const deleteSession = (subjectId, date, slot = "1") => {
    ...
    setSessions((prev) => {
      const next = { ...prev };
      delete next[`${subjectId}__${date}__${slot}`];
      return next;
    });
  };
