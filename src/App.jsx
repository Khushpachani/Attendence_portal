  ...
  seedStudents.forEach((st) => {
    ...
    subs.forEach((sub) => {
      dates.forEach((date) => {
        ...
        rec[key] = roll < 0.18 ? "absent" : "present";
      });
    });
  });
  return rec;
...
export default function AttendancePortal() {
  ...
  const applyServerData = (data) => {
    if (data) {
      ...
      // app was simplified to just Present/Absent) into Absent, so old
      // saved data can't crash code that only knows about the two statuses.
      const rawAttendance = data.attendance || {};
      const cleanAttendance = {};
      Object.entries(rawAttendance).forEach(([key, status]) => {
        cleanAttendance[key] = status === "present" ? "present" : "absent";
      });
    ...
    setAttendance((prev) => {
      ...
      Object.keys(prev).forEach((key) => {
        ...
        if (sid === studentId && subId === fromSubjectId) {
          next[`${studentId}__${toSubjectId}__${date}`] = next[key];
          delete next[key];
        }
      });
      return next;
  ...
  // Wipes attendance for EVERY student for one lecture (subject + date) in
  // one go — e.g. the class was cancelled and shouldn't have been marked.
  const deleteSession = (subjectId, date) => {
    const suffix = `__${subjectId}__${date}`;
    setAttendance((prev) => {
      const next = {};
      Object.entries(prev).forEach(([key, val]) => {
  ...
  // One detailed sheet per subject: dates as columns, one Status cell per
  // student per date (P/A, or blank if unmarked). Session remarks (if any)
  // are listed as footnotes below the table rather than as a column.
  const buildSubjectSheet = (wb, subject, usedNames) => {
    ...
    const totalCols = LEFT.length + dates.length + RIGHT.length;
    ...
    RIGHT.forEach((_, i) => {
      ws.getColumn(LEFT.length + dates.length + i + 1).width = 12;
    });
    ...
    // Rows 2-3 — headers (fixed + summary columns span both rows; each date
    // is one column showing the date and, underneath, its lecture time)
    ...
    RIGHT.forEach((label, i) => {
      const col = LEFT.length + dates.length + i + 1;
      ws.mergeCells(2, col, 3, col);
    ...
    roster.forEach((st, idx) => {
      ...
      const pct = total ? Math.round((present / total) * 100) : 0;
      const rightBase = LEFT.length + dates.length;
    ...
    const remarkDates = dates.filter((d) => (sessions[`${subject.id}__${d}`]?.remark || "").trim());
    if (remarkDates.length > 0) {
      footRow += 1;
      ws.getCell(footRow, 1).value = "Session notes:";
      ws.getCell(footRow, 1).font = { bold: true };
      remarkDates.forEach((d) => {
        footRow += 1;
        const dObj = new Date(d);
        ...
        cell.value = `${label}: ${sessions[`${subject.id}__${d}`].remark}`;
...
function MarkTab({ subjects, studentsFor, attendance, setMark, getSession, setSession }) {
  ...
  const presentCount = roster.filter((st) => attendance[`${st.id}__${subjectId}__${date}`] === "present").length;
  const markedCount = roster.filter((st) => attendance[`${st.id}__${subjectId}__${date}`]).length;
...
function AllAttendanceTab({ students, subjects, attendance, sessions, studentsFor, setMark, setSession, deleteMark, deleteSession }) {
  ...
  const [search, setSearch] = useState("");
  const [editingDate, setEditingDate] = useState(null); // date currently being edited in the session popover
  ...
  const dates = useMemo(() => {
    ...
    Object.keys(attendance).forEach((key) => {
      const [sid, subId, date] = key.split("__");
      if (subId === subject.id && roster.some((r) => r.id === sid)) set.add(date);
    });
    return Array.from(set).sort();
  }, [attendance, subject, roster]);
    ...
    <div>
      ...
      {!subject && (
        <div style={{ color: COLORS.slate, fontSize: 13, textAlign: "center", padding: "40px 0" }}>
          Pick a subject above to see every student's attendance, date by date — just like the exported Excel sheet.
        </div>
      )}
        ...
        <>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14, flexWrap: "wrap", gap: 12 }}>
            <div>
              ...
              <div style={{ fontSize: 12, color: COLORS.slate }}>
                {roster.length} students · {dates.length} lecture{dates.length === 1 ? "" : "s"} recorded
              </div>
            </div>
          ...
          {dates.length === 0 ? (
            <div style={{ color: COLORS.slate, fontSize: 13, textAlign: "center", padding: "30px 0" }}>
              No attendance has been recorded for this subject yet — mark some in the "Mark Attendance" tab first.
            </div>
          ) : (
              ...
              <table style={{ borderCollapse: "collapse", fontSize: 12.5, minWidth: "100%" }}>
                <thead>
                  <tr style={{ background: COLORS.parchmentDark, textAlign: "left" }}>
                    ...
                    {dates.map((d) => {
                      const session = sessions[`${subject.id}__${d}`] || { time: "", remark: "" };
                      return (
                        <th key={d} style={{ ...thStyle, textAlign: "center", minWidth: 68, cursor: "pointer" }} onClick={() => setEditingDate(d)}>
                ...
                <tbody>
                  {filteredRoster.map((st, idx) => {
                    ...
                    return (
                      <tr key={st.id} style={{ borderTop: `1px solid ${COLORS.line}`, background: idx % 2 === 1 ? COLORS.parchment : "#fff" }}>
                        ...
                        {dates.map((d) => {
                          const key = `${st.id}__${subject.id}__${d}`;
                          ...
                          return (
                            <td key={d} style={{ ...tdStyle, textAlign: "center", padding: 4 }}>
                              <button
                                onClick={() => setMark(st.id, subject.id, d, status ? cycleStatus(status) : "present")}
                  ...
                  {filteredRoster.length === 0 && (
                    <tr>
                      <td colSpan={dates.length + 5} style={{ ...tdStyle, textAlign: "center", color: COLORS.slate, padding: 24 }}>
                  ...
                  <tfoot>
                    <tr style={{ borderTop: `2px solid ${COLORS.ink}`, background: COLORS.parchmentDark, fontWeight: 700 }}>
                      <td style={{ ...tdStyle, position: "sticky", left: 0, background: COLORS.parchmentDark }} colSpan={2}>
                        Total Present (that day)
                      </td>
                      {dates.map((d) => {
                        const presentThatDay = filteredRoster.filter(
                          (st) => attendance[`${st.id}__${subject.id}__${d}`] === "present"
                        ).length;
                        return (
                          <td key={d} style={{ ...tdStyle, textAlign: "center", color: pctColor(Math.round((presentThatDay / filteredRoster.length) * 100)) }}>
                            {presentThatDay}/{filteredRoster.length}
                          </td>
                        );
                      })}
          ...
          <div style={{ fontSize: 11, color: COLORS.slate, marginTop: 10 }}>
            Click any status cell to cycle Present → Absent → On Leave → Proxy. Click a date header to edit that lecture's time and remark.
          </div>
        </>
      )}
      ...
      {editingDate && subject && (
        <SessionEditPopover
          subject={subject}
          date={editingDate}
          session={sessions[`${subject.id}__${editingDate}`] || { time: "", remark: "" }}
          onSave={(patch) => setSession(subject.id, editingDate, patch)}
          onDelete={() => deleteSession(subject.id, editingDate)}
...
function SessionEditPopover({ subject, date, session, onSave, onDelete, onClose }) {
  ...
  const handleDelete = () => {
    if (
      !window.confirm(
        `Delete ALL attendance for ${subject.code} on ${new Date(date).toLocaleDateString("en-GB", { day: "2-digit", month: "short", year: "numeric" })}?\n\nThis removes every student's status for this lecture and can't be undone.`
      )
    )
      return;
    onDelete();
  ...
  return (
    ...
    >
      <div onClick={(e) => e.stopPropagation()} style={{ ...cardStyle, width: 360, margin: 0 }}>
        ...
        <div style={{ fontSize: 12, fontWeight: 600, marginBottom: 6 }}>
          Remark <span style={{ color: COLORS.slate, fontWeight: 400 }}>(for this lecture, not one student)</span>
        </div>
        <input
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "space-between", alignItems: "center" }}>
          ...
          <button
            ...
            title="Delete all attendance for this date"
          >
            <X size={14} /> Delete date
          </button>
        </div>
      </div>
    </div>
